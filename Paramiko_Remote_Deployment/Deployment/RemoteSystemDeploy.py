...
for server in serverstocheck:
    ...
    if response == 0:
        ...
        ftp_client.put(script_ftp_file_path + "scheduledeployment.sh", "/tmp/scheduledeployment.sh")
        ftp_client.put(script_ftp_file_path + "deployrepo.sh", "/tmp/deployrepo.sh")
        ftp_client.close()

        command = "sudo -S chmod u+x /tmp/scheduledeployment.sh"
        stdin, stdout, stderr = client.exec_command(command)
        stdin.write(password + "\n")
        print(stdout.read().decode("utf-8"))

        command = "sudo -S chmod u+x /tmp/deployrepo.sh"
        stdin, stdout, stderr = client.exec_command(command)
        stdin.write(password + "\n")
        print(stdout.read().decode("utf-8"))
        ...
        print(":=============:")
        print("------------------------------------------------")
        command = "sudo -S /tmp/./scheduledeployment.sh"
        stdin, stdout, stderr = client.exec_command(command)
        stdin.write(password + "\n")
        print(stdout.read().decode("utf-8"))

        stdin, stdout, stderr = client.exec_command("rm /tmp/scheduledeployment.sh")
        print(stdout.read().decode("utf-8"))
