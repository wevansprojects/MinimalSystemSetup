...
for server in serverstocheck:
    ...
    if response == 0:
        ...
        ftp_client = client.open_sftp()
        ftp_client.put(
            "pyscript/schedulesystemupdate.sh", "/tmp/schedulesystemupdate.sh"
        )
        ftp_client.put("pyscript/updatesystem.sh", "/tmp/updatesystem.sh")
        ftp_client.close()
        ...
        print("\n")
        # command = "sudo -S cat /var/log/dnf.log|tail -10"
